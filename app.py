...
with st.sidebar:
    ...
    llms_ok      = groq_ok or anthropic_ok
    ...
    st.markdown(f'<div class="kpi-card" style="--ac:{c2};padding:10px 14px;margin-bottom:8px;"><div class="kpi-label">LLMs</div><div style="font-size:.75rem;color:{c2};">{"[OK] Prontos" if llms_ok else "[!] Nao configurado"}</div><div class="kpi-sub">{" | ".join(llm_sub)}</div></div>', unsafe_allow_html=True)

    # Input de API Keys (quando nao configuradas)
    if not llms_ok:
        st.divider()
        st.markdown(f'<div style="font-size:.63rem;color:{WARN};font-family:IBM Plex Mono,monospace;margin-bottom:8px;text-transform:uppercase;letter-spacing:1px;">Configurar API Keys</div>', unsafe_allow_html=True)
        gk = st.text_input("GROQ_API_KEY (gratis)", type="password", placeholder="gsk_...")
        ak = st.text_input("ANTHROPIC_API_KEY", type="password", placeholder="sk-ant-...")
        if st.button("Aplicar", type="primary", use_container_width=True):
            ...
            st.success("Keys salvas!")
            st.rerun()
        st.markdown(f'<div style="font-size:.6rem;color:{TXT3};margin-top:4px;">Groq gratis: console.groq.com<br>Anthropic: console.anthropic.com</div>', unsafe_allow_html=True)
