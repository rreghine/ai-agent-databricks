...
section[data-testid="stSidebar"] .block-container{{background:{SURF} !important;}}
section[data-testid="stSidebar"] *{{color:{TXT} !important;}}
section[data-testid="stSidebar"] input{{background:{SURF2} !important;border:1px solid {BORDER} !important;color:{TXT} !important;}}

/* ── Dataframe/tabela fundo escuro ── */
.stDataFrame{{background:{SURF} !important;border:1px solid {BORDER};border-radius:8px;}}
.stDataFrame iframe{{background:{SURF} !important;}}
[data-testid="stDataFrame"]{{background:{SURF} !important;}}
[data-testid="stDataFrame"] *{{background:{SURF} !important;color:{TXT} !important;}}
.dvn-scroller{{background:{SURF} !important;}}
.col_heading{{background:{SURF2} !important;color:{TXT2} !important;font-size:.7rem !important;text-transform:uppercase;letter-spacing:.5px;}}
.data{{background:{SURF} !important;color:{TXT} !important;font-size:.78rem !important;}}
.row_heading{{background:{SURF2} !important;color:{TXT3} !important;}}
.blank{{background:{SURF2} !important;}}

/* ── Tabs fundo escuro ── */
.stTabs [data-baseweb="tab-list"]{{background:{SURF} !important;border-bottom:1px solid {BORDER};gap:4px;padding:0 4px;}}
.stTabs [data-baseweb="tab"]{{background:transparent !important;color:{TXT2} !important;border-radius:6px 6px 0 0;font-size:.82rem;padding:8px 16px;}}
.stTabs [aria-selected="true"]{{background:{SURF2} !important;color:{TXT} !important;border-bottom:2px solid {ACCENT3} !important;}}
.stTabs [data-baseweb="tab-panel"]{{background:{BG} !important;padding-top:16px;}}

/* ── Inputs e chat fundo escuro ── */
.stChatInput textarea{{background:{SURF2} !important;border:1px solid {BORDER} !important;color:{TXT} !important;border-radius:10px !important;}}
.stChatInput button{{background:{ACCENT} !important;border-radius:8px !important;}}
.stTextInput input{{background:{SURF2} !important;border:1px solid {BORDER} !important;color:{TXT} !important;}}
.stTextInput label{{color:{TXT2} !important;font-size:.7rem !important;}}

/* ── Expander fundo escuro ── */
.streamlit-expanderHeader{{background:{SURF2} !important;border:1px solid {BORDER} !important;color:{TXT2} !important;border-radius:6px !important;}}
.streamlit-expanderContent{{background:{SURF} !important;border:1px solid {BORDER} !important;border-top:none !important;}}

/* ── Botoes ── */
.stButton button{{background:{SURF2} !important;border:1px solid {BORDER} !important;color:{TXT} !important;border-radius:8px !important;font-size:.78rem !important;transition:all .2s;}}
.stButton button:hover{{background:{ACCENT}22 !important;border-color:{ACCENT} !important;color:{TXT} !important;}}
.stButton [data-testid="baseButton-primary"]{{background:{ACCENT} !important;border:none !important;color:white !important;}}
/* ── Selects ── */
[data-baseweb="select"] *{{background:{SURF2} !important;color:{TXT} !important;border-color:{BORDER} !important;}}
[data-baseweb="popover"] li{{background:{SURF2} !important;color:{TXT} !important;}}
[data-baseweb="popover"] li:hover{{background:{ACCENT}33 !important;}}
[role="listbox"] *{{background:{SURF2} !important;color:{TXT} !important;}}
.stSelectbox label{{color:{TXT2} !important;font-size:.72rem !important;}}
/* ── Sidebar ── */
section[data-testid="stSidebar"]{{background:{SURF} !important;border-right:1px solid {BORDER};}}
section[data-testid="stSidebar"] *{{color:{TXT} !important;}}
section[data-testid="stSidebar"] input{{background:{SURF2} !important;border:1px solid {BORDER} !important;}}
section[data-testid="stSidebar"] label{{color:{TXT2} !important;font-size:.72rem !important;}}
/* ── Code ── */
.stCodeBlock,pre{{background:{SURF2} !important;color:{TXT} !important;border:1px solid {BORDER} !important;border-radius:8px !important;}}
/* ── Alerts ── */
.stAlert,.stAlert *{{background:{SURF2} !important;color:{TXT} !important;border:1px solid {BORDER} !important;}}
/* ── Spinner ── */
...
with tab2:
    ...
    for _, row in df_show.iterrows():
        ...
        cells = "".join([f'<td style="padding:6px 10px;border-bottom:1px solid {BORDER};font-size:.75rem;">{v if not isinstance(v,float) else f"{v:.1f}"}</td>' for v in row])
        rows_html += f'<tr style="background:{bg}">{cells}</tr>'

    headers_html = "".join([f'<th style="padding:6px 10px;background:{SURF2};color:{TXT2};font-size:.62rem;text-transform:uppercase;letter-spacing:.5px;border-bottom:2px solid {BORDER};font-weight:600;">{c}</th>' for c in df_show.columns])
...
with tab3:
    ...
    if st.session_state.get("bench"):
        ...
        for col, llm_n in zip(cols_llm, all_llms):
            ...
            color  = ACCENT if "claude" in llm_n else ACCENT2 if "gpt" in llm_n else SUCCESS
            with col:
                st.markdown(f"""
