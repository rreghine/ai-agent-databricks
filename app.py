...
def get_llm(complexity):
    ...
    # Routing: simple -> Llama (gratis) | medium -> Llama 70B (gratis) | complex -> melhor disponivel
    if complexity == "simple" and gk:
        return ChatGroq(model="llama-3.1-8b-instant",    api_key=gk), "llama-3.1-8b-instant"
    if complexity == "medium" and gk:
        return ChatGroq(model="llama-3.3-70b-versatile", api_key=gk), "llama-3.3-70b-versatile"
    # Fallback complex: Claude > GPT-4o > GPT-4o-mini > Llama 70B
    if ak:
        return ChatAnthropic(model="claude-sonnet-4-6", api_key=ak, max_tokens=512), "claude-sonnet-4-6"
    if ok and OPENAI_OK:
        return ChatOpenAI(model="gpt-4o", api_key=ok, max_tokens=512), "gpt-4o"
    if ok and OPENAI_OK:
        return ChatOpenAI(model="gpt-4o-mini", api_key=ok, max_tokens=512), "gpt-4o-mini"
    if gk:
        return ChatGroq(model="llama-3.3-70b-versatile", api_key=gk), "llama-3.3-70b-versatile"
    return None, None
...
/* ── Code block ── */
.stCodeBlock{{background:{SURF2} !important;border:1px solid {BORDER} !important;border-radius:8px !important;}}
pre{{background:{SURF2} !important;color:{TXT} !important;}}
